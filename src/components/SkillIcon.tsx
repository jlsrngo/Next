    ...
    return (
        <div className="group flex flex-col items-center w-14 md:w-20 gap-1 md:gap-2 cursor-pointer pt-2">
            <div className="relative w-12 h-12 md:w-16 md:h-16 transition-all duration-300 z-10 block">
                <div
                    className={`skill-icon-backdrop absolute inset-0 rounded-2xl transition-all duration-300 ease-out translate-x-2 -translate-y-2 rotate-[15deg] group-hover:translate-x-6 group-hover:-translate-y-6 group-hover:rotate-[35deg] bg-gradient-to-br ${gradient}`}
                    style={{ willChange: 'transform' }}
                />
                <div className="skill-icon-surface absolute inset-0 rounded-2xl bg-white/50 dark:bg-white/15 border border-slate-200 dark:border-white/10 flex items-center justify-center transition-transform duration-300 z-10 group-hover:scale-105">
                    {IconComponent ? (
